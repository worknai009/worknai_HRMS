...
app.use(
  cors({
    origin: [
      "http://localhost:3000",
      "http://localhost:5173",
      "https://worknaihrms.online",
      "http://worknaihrms.online",
    ],
    credentials: true,
...
connectDB().then(async () => {
  ...
  try {
    ...
    if (badIndex) {
      console.log(
        "⚠️ Found conflicting index 'userId_1_date_1'. Dropping it..."
      );
      await collection.dropIndex("userId_1_date_1");
      console.log(
        "✅ Successfully removed duplicate index. Server is now safe."
      );
    }
  } catch (err) {
