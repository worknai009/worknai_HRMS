...
const SuperAdminDashboard = () => {
  ...
  const handleLimitAction = async (companyId, action) => {
    try {
      ...
      toast.success(
        action === "approve" ? "Limit Increased ✅" : "Rejected ❌"
      );
      fetchData();
    } catch {
  ...
  const handleRejectOrDelete = async (id, type) => {
    if (!window.confirm("⚠️ Confirm Deletion/Rejection? This is irreversible."))
      return;
    try {
      const endpoint =
        type === "reject"
          ? `/superadmin/inquiry/${id}`
  ...
  const submitApproval = async () => {
    ...
    try {
      await API.post("/superadmin/approve-inquiry", {
    ...
    <div className="sa-dashboard">
      ...
      <div className="sa-container">
          ...
          <div className="tabs">
            ...
            >
              ...
              {pendingInquiries.length > 0 && (
                <span className="badge">{pendingInquiries.length}</span>
              )}
            </button>
            <button
            ...
            >
              ...
              {limitRequests.length > 0 && (
                <span className="badge red">{limitRequests.length}</span>
              )}
            </button>
          </div>
        ...
        <div className="data-grid">
          {loading ? (
            <div className="state-msg">Loading Data...</div>
          ) : filtered.length === 0 ? (
            <div className="state-msg">No records found.</div>
          ) : (
            filtered.map((item) => (
              <div key={item._id} className="data-card">
                <div className="card-head">
                  <div className="avt">
                    {(item.name || item.companyName || "C").charAt(0)}
                  </div>
                  <div className="meta">
                    <h4>{item.name || item.companyName}</h4>
                    <small>{item.email}</small>
                  </div>
                  <span className={`status-pill ${activeTab}`}>
                ...
                <div className="card-body">
                  <p>
                    <FaPhone /> {item.mobile || "N/A"}
                  </p>
                  <p>
                    <FaMapMarkerAlt />{" "}
                    {item.location?.address || item.address || "No Address"}
                  </p>
                  {activeTab === "requests" && (
                    <p className="alert-text">
                      ⚠️ Requesting HR Limit Increase
                    </p>
                  )}
                </div>
        ...
        <div className="modal-overlay">
          <div className="modal-box">
            ...
            <div className="modal-actions">
              ...
              <button className="solid" onClick={submitEdit}>
                Save Changes
              </button>
            </div>
          </div>
